...
Closed-system residuals are checked near numerical precision. V5 conserves heat to 1.537e-9 W, uses no fallback, and repeats with identical output hashes, so the remaining temperature error is not balance leakage or stochastic noise. Replacing the vertical-door shortcut with the verified NIST two-opening equation lowers kitchen RMSE from 3.737 to 3.623 degC and maximum pooled error from 8.565 to 7.783 degC, but pooled RMSE remains 2.626 degC. The error is still dominated by positive kitchen excursions during roughly 1.8 kW heat pulses and by drift from 1.525 degC in the first scored week to 3.466 degC in the last, which points to the one-mass-node construction form and longer-timescale boundary/forcing error rather than another doorway coefficient.

## Limitations

The canonical object path represents heater split, supply-air temperature, per-opening state, component properties, thermal bridges, the time-varying cellar boundary, and temperature/pressure-driven vertical-door exchange. It still collapses all construction layers in each air body into one mass state, uses a single air-to-mass transfer coefficient, uses generic closed-blind optics, applies the source whole-house infiltration estimate uniformly by zone, has no whole-building pressure solve, and retains prescribed exchange for the horizontal attic hatch. Source uncertainty is registered but not propagated into weighted scores; mean radiant/mass temperature is unmeasured.
